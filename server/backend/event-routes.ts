...
import db, { formatDate, getStartOfDay } from "./database";
...
router.get("/all-filtered", (req: Request, res: Response) => {
  try {
    ...
    // filter by search label (using regex) on any value
    if (qFilter.search) {
    ...
    for (let i = dayZeroDate.getTime(); i <= today; i += weekInMilliseconds) {
      ...
      for (let j = i + weekInMilliseconds; j <= today; j += weekInMilliseconds) {
        const loginArr: Event[] = allEvents.filter(
          (event) =>
            event.name === "login" && event.date >= j && event.date < j + weekInMilliseconds
        );
